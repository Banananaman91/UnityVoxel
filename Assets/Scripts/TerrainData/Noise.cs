...
namespace TerrainData
{
    public static class Noise
    {
        ...
        public static float[,] GenerateNoiseMap(int width, int height, float scale, int octaves, float lacunarity, float amplitude, float frequency, int seed, float groundLevel, Vector2 viewPos)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    noiseMap[x, y] = GenerateSample(new float3(x, y, 0), scale, seed, groundLevel, viewPos, octaves, lacunarity, amplitude, frequency, false);
                }
            }
        ...
        public static float Generate2DNoiseValue(float x, float y, float scale, int octaves, float lacunarity, float amplitude, float frequency, int seed, float groundLevel)
        {
            return GenerateSample(new float3(x, y, 0), scale, seed, groundLevel, Vector2.zero, octaves, lacunarity, amplitude, frequency, false);
        }
        ...
        public static float Generate3DNoiseValue(float x, float y, float z, float scale, int octaves, float lacunarity, float amplitude, float frequency, int seed, float groundLevel)
        {
            return GenerateSample(new float3(x, y, z), scale, seed, groundLevel, Vector2.zero, octaves, lacunarity, amplitude, frequency, true);
        }
        ...
        private static float GenerateSample(float3 coords, float scale, int seed,  float groundLevel, Vector2 viewPos, int octaves, float lacunarity, float amplitude, float frequency, bool threeDimensions)
        {
            float noiseReturn = 0;
